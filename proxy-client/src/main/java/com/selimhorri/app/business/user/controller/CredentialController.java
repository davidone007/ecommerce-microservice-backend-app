...
public class CredentialController {
	...
	public ResponseEntity<CredentialDto> save(@RequestBody final CredentialDto credentialDto,
			HttpServletRequest request, @AuthenticationPrincipal UserDetails userDetails) {
		// Allow unauthenticated registration: if no authenticated user info is present,
		// skip owner/admin check so new users can create their credentials after user creation.
		if (credentialDto == null || credentialDto.getUserDto() == null
				|| credentialDto.getUserDto().getUserId() == null) {
			throw new IllegalArgumentException("Credential must include a user with userId");
		}

		final Object authUserIdAttr = request.getAttribute("userId");
		if (userDetails != null && authUserIdAttr != null) {
			// authenticated caller: enforce owner/admin check
			authUtil.canActivate(request, credentialDto.getUserDto().getUserId().toString(), userDetails);
		} else {
			// unauthenticated caller: allow registration flow (creating credentials for existing user)
			// Note: user-service will still validate that the user exists and that username is unique.
		}
